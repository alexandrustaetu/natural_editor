...
void Letter::create_letter_faces(const char letter) {
    ...
    GLfloat font_height = (g->face->bbox.yMax - g->face->bbox.yMin ) / TEXT_SIZE;

    GLfloat letter_offsety = (this->height - g->metrics.horiBearingY) / TEXT_SIZE;

    this->offsety = ( font_height - this->height  ) /2 + font_height ;
    ...
    this->scene.setUp(&this->letter_points);
}
