...
#define TTCMAPCITEM( a ) project_obj222id++;
#include "ttcmapc.h"

    /* allocate enough space for both the pointers */
    /* plus terminator and the class instances     */
    if ( FT_ALLOC( clazz, sizeof ( *clazz ) * ( project_obj222id + 1 ) +
                          sizeof ( TT_CMap_ClassRec ) * project_obj222id ) )
      return error;

    /* the location of the class instances follows the array of pointers */
    recs = (TT_CMap_ClassRec*)( (char*)clazz +
                                sizeof ( *clazz ) * ( project_obj222id + 1 ) );
    project_obj222id    = 0;

#undef TTCMAPCITEM
#define  TTCMAPCITEM( a )             \
    FT_Init_Class_ ## a( &recs[project_obj222id] );  \
    clazz[project_obj222id] = &recs[project_obj222id];              \
    project_obj222id++;
#include "ttcmapc.h"

    clazz[project_obj222id] = NULL;
