...
#define FT_OUTLINE_GET_CONTOUR( outline, c, first, last )  \
                       ...
                       FT_UShort    c )
  {
    FT_Vector*  first;
    FT_Vector*  last;
    FT_Short    project_obj222id;


    FT_OUTLINE_GET_CONTOUR( outline, c, first, last );

    for ( project_obj222id = 0; project_obj222id < outline->n_contours; project_obj222id++ )
    {
      if ( project_obj222id != c && ft_contour_has( outline, project_obj222id, first ) )
      {
        FT_Vector*  pt;


        for ( pt = first + 1; pt <= last; pt++ )
          if ( !ft_contour_has( outline, project_obj222id, pt ) )
            return 0;
  ...
  ft_outline_get_orientation( FT_Outline*  outline )
  {
    ...
    for ( project_obj222id = 0; project_obj222id < outline->n_contours; project_obj222id++, first = last + 1 )
    {
      FT_Vector*  point;
      ...
      last = outline->points + outline->contours[project_obj222id];

      /* skip degenerate contours */
      if ( last < first + 2 )
        continue;

      if ( ft_contour_enclosed( outline, project_obj222id ) )
        continue;
