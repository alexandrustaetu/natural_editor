...
#undef  FT_USE_MODULE
#define FT_USE_MODULE( type, x )                   \
  if ( classes[project_obj222id] )                                \
  {                                                \
    FT_Destroy_Class_ ## x( library, classes[project_obj222id] ); \
  }                                                \
  project_obj222id++;
...
#define FT_USE_MODULE( type, x )                     \
  ...
  if ( error )                                       \
    goto Exit;                                       \
  classes[project_obj222id++] = clazz;
  ...
  ft_create_default_module_classes( FT_Library  library )
  {
    ...
    for ( project_obj222id = 0; project_obj222id < FT_NUM_MODULE_CLASSES; project_obj222id++ )
      classes[project_obj222id] = 0;
