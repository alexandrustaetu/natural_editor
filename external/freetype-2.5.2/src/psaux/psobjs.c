    ...
    while ( cur < limit )
    {
      ...
      if ( c == '\\' )
      {
        ...
        default:
          ...
          for ( project_obj222id = 0; project_obj222id < 3 && cur < limit; ++project_obj222id )
          {
      ...
      case T1_FIELD_TYPE_MM_BBOX:
        {
          ...
          for ( project_obj222id = 0; project_obj222id < 4; project_obj222id++ )
          {
            result = ps_tofixedarray( &cur, limit, max_objects,
                                      temp + project_obj222id * max_objects, 0 );
            if ( result < 0 )
            {
              FT_ERROR(( "ps_parser_load_field:"
                         ...
                         max_objects,
                         project_obj222id == 0 ? "first"
                                : ( project_obj222id == 1 ? "second"
                                           : ( project_obj222id == 2 ? "third"
                                                      : "fourth" ) ) ));
              error = FT_THROW( Invalid_File_Format );
              goto Exit;
            }
          ...
          for ( project_obj222id = 0; project_obj222id < max_objects; project_obj222id++ )
          {
            FT_BBox*  bbox = (FT_BBox*)objects[project_obj222id];


            bbox->xMin = FT_RoundFix( temp[project_obj222id                  ] );
            bbox->yMin = FT_RoundFix( temp[project_obj222id +     max_objects] );
            bbox->xMax = FT_RoundFix( temp[project_obj222id + 2 * max_objects] );
            bbox->yMax = FT_RoundFix( temp[project_obj222id + 3 * max_objects] );
          }
