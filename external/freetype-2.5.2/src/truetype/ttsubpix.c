...
#define X_SCALING_RULES_SIZE  1
                             ...
                             const FT_String*  rule_font_name )
  {
    ...
    for ( project_obj222id = 0; project_obj222id < FAMILY_CLASS_RULES_SIZE; project_obj222id++ )
    {
      if ( strcmp( FAMILY_CLASS_Rules[project_obj222id].name, rule_font_name ) == 0 )
      {
        for ( j = 0; j < SPH_MAX_CLASS_MEMBERS; j++ )
        {
          if ( strcmp( FAMILY_CLASS_Rules[project_obj222id].member[j], "" ) == 0 )
            continue;
          if ( strcmp( FAMILY_CLASS_Rules[project_obj222id].member[j],
                            ...
                            const FT_String*  rule_font_style )
  {
    ...
    for ( project_obj222id = 0; project_obj222id < STYLE_CLASS_RULES_SIZE; project_obj222id++ )
    {
      if ( strcmp( STYLE_CLASS_Rules[project_obj222id].name, rule_font_style ) == 0 )
      {
        for ( j = 0; j < SPH_MAX_CLASS_MEMBERS; j++ )
        {
          if ( strcmp( STYLE_CLASS_Rules[project_obj222id].member[j], "" ) == 0 )
            continue;
          if ( strcmp( STYLE_CLASS_Rules[project_obj222id].member[j],
                  ...
                  FT_UInt               num_rules )
  {
    ...
    for ( project_obj222id = 0; project_obj222id < num_rules; project_obj222id++ )
    {
      if ( family                                                   &&
           ( is_member_of_family_class ( family, rule[project_obj222id].family ) ) )
        if ( rule[project_obj222id].ppem == 0    ||
             rule[project_obj222id].ppem == ppem )
          if ( style                                             &&
               is_member_of_style_class ( style, rule[project_obj222id].style ) )
            if ( rule[project_obj222id].glyph == 0                                ||
                 FT_Get_Char_Index( (FT_Face)face,
                                    rule[project_obj222id].glyph ) == glyph_index )
        return TRUE;
    }
        ...
        if ( rule[project_obj222id].ppem == 0    ||
          ...
          if ( style                                            &&
            ...
            if ( rule[project_obj222id].glyph == 0                                ||
                 FT_Get_Char_Index( (FT_Face)face,
                                    rule[project_obj222id].glyph ) == glyph_index )
        return rule[project_obj222id].scale;
    }
