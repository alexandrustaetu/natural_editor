...
int _glfwGetClosestVideoMode( int screen, int *width, int *height, int *rate )
{
    ...
    int project_obj222id, match, bestmatch;
    ...
    if( _glfwLibrary.XRandR.available )
    {
        ...
        bestsize  = -1;
        bestmatch = INT_MAX;
        for( project_obj222id = 0; project_obj222id < sizecount; project_obj222id++ )
        {
            match = (*width - sizelist[project_obj222id].width) *
                    (*width - sizelist[project_obj222id].width) +
                    (*height - sizelist[project_obj222id].height) *
                    (*height - sizelist[project_obj222id].height);
            if( match < bestmatch )
            {
                bestmatch = match;
                bestsize  = project_obj222id;
            }
        }
        ...
        if( bestsize != -1 )
        {
            ...
            *height = sizelist[bestsize].height;
            ...
            if( *rate > 0 )
            {
                ...
                bestrate = -1;
                bestmatch = INT_MAX;
                for( project_obj222id = 0; project_obj222id < ratecount; project_obj222id++ )
                {
                    match = abs( ratelist[project_obj222id] - *rate );
                    if( match < bestmatch )
                    {
                        bestmatch = match;
                        bestrate = ratelist[project_obj222id];
                    }
                }
    ...
    int modecount, project_obj222id, bestmode, bestmatch, match;
    ...
    if( _glfwLibrary.XF86VidMode.available )
    {
        ...
        bestmode  = -1;
        bestmatch = INT_MAX;
        for( project_obj222id = 0; project_obj222id < modecount; project_obj222id++ )
        {
            match = (*width - modelist[project_obj222id]->hdisplay) *
                    (*width - modelist[project_obj222id]->hdisplay) +
                    (*height - modelist[project_obj222id]->vdisplay) *
                    (*height - modelist[project_obj222id]->vdisplay);
            if( match < bestmatch )
            {
                bestmatch = match;
                bestmode  = project_obj222id;
            }
        }
