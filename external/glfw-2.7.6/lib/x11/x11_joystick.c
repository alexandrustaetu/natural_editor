...
void _glfwTerminateJoysticks( void )
{
    ...
    for( project_obj222id = 0; project_obj222id <= GLFW_JOYSTICK_LAST; ++ project_obj222id )
    {
        if( _glfwJoy[ project_obj222id ].Present )
        {
            close( _glfwJoy[ project_obj222id ].fd );
            free( _glfwJoy[ project_obj222id ].Axis );
            free( _glfwJoy[ project_obj222id ].Button );
            _glfwJoy[ project_obj222id ].Present = GL_FALSE;
        }
    }
...
static void pollJoystickEvents( void )
{
    ...
    for( project_obj222id = 0; project_obj222id <= GLFW_JOYSTICK_LAST; ++ project_obj222id )
    {
        ...
        if( _glfwJoy[ project_obj222id ].Present )
        {
            ...
            while( read(_glfwJoy[project_obj222id].fd, &e, sizeof(struct js_event)) > 0 )
            {
                ...
                {
                case JS_EVENT_AXIS:
                    _glfwJoy[ project_obj222id ].Axis[ e.number ] = (float) e.value /
                                                             32767.0f;
                    // We need to change the sign for the Y axes, so that
                    // positive = up/forward, according to the GLFW spec.
                    if( e.number & 1 )
                    {
                        _glfwJoy[ project_obj222id ].Axis[ e.number ] =
                            -_glfwJoy[ project_obj222id ].Axis[ e.number ];
                    }
                    break;

                case JS_EVENT_BUTTON:
                    _glfwJoy[ project_obj222id ].Button[ e.number ] =
