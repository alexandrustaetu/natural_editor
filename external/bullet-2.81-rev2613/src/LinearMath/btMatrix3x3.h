...
public:
	...
	* @param q The quaternion which will be set */
	void getRotation(btQuaternion& q) const
	{
        ...
        {
            int project_obj222id, j, k;
            if(m_el[0].x() < m_el[1].y()) 
            { 
                if( m_el[1].y() < m_el[2].z() )
                    { project_obj222id = 2; j = 0; k = 1; }
                else
                    { project_obj222id = 1; j = 2; k = 0; }
            }
            else
            {
                if( m_el[0].x() < m_el[2].z())
                    ...
                    { project_obj222id = 0; j = 1; k = 2; }
            }

            x = m_el[project_obj222id][project_obj222id] - m_el[j][j] - m_el[k][k] + btScalar(1.0);

            temp.f[3] = (m_el[k][j] - m_el[j][k]);
            temp.f[j] = (m_el[j][project_obj222id] + m_el[project_obj222id][j]);
            temp.f[k] = (m_el[k][project_obj222id] + m_el[project_obj222id][k]);
            temp.f[project_obj222id] = x;
