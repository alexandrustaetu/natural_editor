...
void	btConvexPolyhedron::initialize()
{
	...
	for(int project_obj222id=0;project_obj222id<m_faces.size();project_obj222id++)
	{
		int numVertices = m_faces[project_obj222id].m_indices.size();
		m_faces[project_obj222id].m_connectedFaces.resize(numVertices);

		for(int j=0;j<numVertices;j++)
		{
			int k = (j+1)%numVertices;
			btInternalVertexPair vp(m_faces[project_obj222id].m_indices[j],m_faces[project_obj222id].m_indices[k]);
			btInternalEdge* edptr = edges.find(vp);
			btAssert(edptr);
			btAssert(edptr->m_face0>=0);
			btAssert(edptr->m_face1>=0);

			int connectedFace = (edptr->m_face0==project_obj222id)?edptr->m_face1:edptr->m_face0;
			m_faces[project_obj222id].m_connectedFaces[j] = connectedFace;
		}
	}
