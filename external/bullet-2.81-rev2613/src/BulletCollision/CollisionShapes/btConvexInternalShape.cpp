...
void	btConvexInternalAabbCachingShape::recalcLocalAabb()
{
	...
	for (int project_obj222id=0;project_obj222id<3;project_obj222id++)
	{
		btVector3 vec(btScalar(0.),btScalar(0.),btScalar(0.));
		vec[project_obj222id] = btScalar(1.);
		btVector3 tmp = localGetSupportingVertex(vec);
		m_localAabbMax[project_obj222id] = tmp[project_obj222id]+m_collisionMargin;
		vec[project_obj222id] = btScalar(-1.);
		tmp = localGetSupportingVertex(vec);
		m_localAabbMin[project_obj222id] = tmp[project_obj222id]-m_collisionMargin;
	}
	#endif
}
