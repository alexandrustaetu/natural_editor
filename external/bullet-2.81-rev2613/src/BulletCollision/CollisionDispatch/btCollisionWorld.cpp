...
#else
	for (int project_obj222id=0;project_obj222id<this->getNumCollisionObjects();project_obj222id++)
	{
		rayCB.process(m_collisionObjects[project_obj222id]->getBroadphaseHandle());
	}	
...
void	btCollisionWorld::convexSweepTest(const btConvexShape* castShape, const btTransform& convexFromWorld, const btTransform& convexToWorld, ConvexResultCallback& resultCallback, btScalar allowedCcdPenetration) const
{
	...
	for (project_obj222id=0;project_obj222id<m_collisionObjects.size();project_obj222id++)
	{
		btCollisionObject*	collisionObject= m_collisionObjects[project_obj222id];
		//only perform raycast if filterMask matches
