...
btScalar btSequentialImpulseConstraintSolver::solveGroupCacheFriendlySetup(btCollisionObject** bodies, int numBodies, btPersistentManifold** manifoldPtr, int numManifolds,btTypedConstraint** constraints,int numConstraints,const btContactSolverInfo& infoGlobal,btIDebugDraw* debugDrawer,btStackAlloc* stackAlloc)
{
	...
	for (int project_obj222id=0;project_obj222id<numConstraints;project_obj222id++)
	{
		btTypedConstraint* constraint = constraints[project_obj222id];
    ...
    for (int project_obj222id=0;project_obj222id<numManifolds;project_obj222id++)
    {
        if (!manifoldPtr[project_obj222id]->getBody0()->isStaticOrKinematicObject())
        {
            bool found=false;
            for (int b=0;b<numBodies;b++)
            {
                
                if (manifoldPtr[project_obj222id]->getBody0()==bodies[b])
                {
                    found = true;
                    break;
                }
        ...
        if (!manifoldPtr[project_obj222id]->getBody1()->isStaticOrKinematicObject())
        {
            bool found=false;
            for (int b=0;b<numBodies;b++)
            {
                if (manifoldPtr[project_obj222id]->getBody1()==bodies[b])
                {
                    found = true;
                    break;
                }
